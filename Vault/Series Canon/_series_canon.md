# Series Canon — filing band

The content of *this* series — what Astraea Chronicles IS. Series-specific; the canon every other band serves. Changes when the story / world / characters develop.

Contents:
- `00_north_star` — series bible + canonical supremacy authority (band anchor)
- `Characters/` — character hubs: stable core identity, one file per character (per-novel arc-state lives as spokes in the Novels band)
- `Encyclopedia/` — worldbuilding reference (the World step's home)
- `Images/` — character / world art supporting the text

## Deferred inner structure
